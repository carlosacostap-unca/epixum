...
import { enrollTeacher, removeTeacherFromCourse } from '@/app/actions/courses'
...
interface CourseTeacherManagementProps {
    initialTeachers: StaffMember[]
    courseId: string
    institutionId: string
}

export default function CourseTeacherManagement({ 
    initialTeachers, 
    courseId, 
    institutionId 
}: CourseTeacherManagementProps) {
    ...
    async function handleAdd(e: React.FormEvent) {
        ...
        try {
            const result = await enrollTeacher(courseId, email)
            ...
            if (result.success) {
                ...
                alert(result.error || 'Error al asignar docente')
            }
        } catch (error) {
    ...
    async function handleRemove(emailToRemove: string) {
        ...
        try {
            const result = await removeTeacherFromCourse(courseId, emailToRemove)
            if (result.success) {
                window.location.reload()
            } else {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-neutral-900 p-4 rounded-lg border border-neutral-800">
                <h2 className="text-xl font-semibold text-gray-200">Docentes Asignados</h2>
                <button 
            ...
            {isAdding && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
                    <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-lg w-full max-w-md">
                        <h3 className="text-xl font-bold text-gray-100 mb-4">Asignar Docente</h3>
